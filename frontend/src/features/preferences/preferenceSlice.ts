...
const preferenceSlice = createSlice({
  ...
  initialState: {
    ...
    yHeroPosition: 0,
    sortingDate: {
      sortingYear: today.getFullYear(),
      sortingMonth: today.getMonth() + 1,
      sortingDay: today.getDate()
    },
    sortingKeywords: {
      keywordMain: '',
      keywordOne: '',
  ...
  reducers: {
    ...
    changeResultsDetailView (state) {
      if (state.resultsDetailView === true) {
        state.resultsDetailView = false
      } else if (state.resultsDetailView === false) {
        state.resultsDetailView = true
      }
    },
    changeFragmentsDetailView (state) {
      if (state.fragmentsDetailView === true) {
        state.fragmentsDetailView = false
      } else if (state.fragmentsDetailView === false) {
        state.fragmentsDetailView = true
      }
    },
    ...
    sortFragmentsBySourceEditOne (state) {
      state.sortFragmentsBySource.KrajowaInformacjaSkarbowa =
        !state.sortFragmentsBySource.KrajowaInformacjaSkarbowa
    },
    sortFragmentsBySourceEditTwo (state) {
      state.sortFragmentsBySource.IzbaSkarbowa =
        !state.sortFragmentsBySource.IzbaSkarbowa
    },
    sortFragmentsBySourceEditThree (state) {
      state.sortFragmentsBySource.MinisterFinansów =
        !state.sortFragmentsBySource.MinisterFinansów
    },
...
export const {
  ...
  sortingDateEdit,
  sortingKeywordsEdit,
  sortingKeywordMainEdit,
  sortingOptionEdit,
  highlightQueryEdit,
  fragmentScrolledEdit,
  preferedSchemeEdit,
  changeResultsDetailView,
  changeFragmentsDetailView,
  editYHeroPosition
